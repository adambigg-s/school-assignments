...
struct GameState deserialize(uint64_t buffer) {
...
void move_up(struct GameState *state) {
    if (state->empty_row == 3)
        return;
    state->tiles[state->empty_row][state->empty_col] = state->tiles[state->empty_row + 1][state->empty_col];
...
void move_left(struct GameState *state) {
    if (state->empty_col == 3)
        return;
    state->tiles[state->empty_row][state->empty_col] = state->tiles[state->empty_row][state->empty_col + 1];
...
// static const uint64_t translator[16] = {1, 1, 2, 6, 24, 120, 720, 5040, 40320, 362880, 3628800, 39916800,
